...
<template>
  <form :class="mode === 'mobile' ? 'flex w-full justify-between gap-3 bg-base-100 px-4 pt-4 border-t border-t-zinc-200' : 'flex gap-2 border-b border-base-200 px-4 py-2.5'" @submit.prevent="submit">
    <div class="relative flex-1"><input :value="modelValue" :class="mode === 'mobile' ? 'input w-full pr-10' : 'input w-full pr-10'" aria-label="Ville ou adresse" autocomplete="street-address" placeholder="Ville ou adresse" @input="emit('update:modelValue', ($event.target as HTMLInputElement).value)" />
    ...
    <button :class="mode === 'mobile' ? 'btn btn-primary btn-square' : 'btn btn-primary btn-square'" :disabled="loading" aria-label="Chercher"><PhMagnifyingGlass :size="mode === 'mobile' ? 20 : 22" /></button>
  </form>
</template>
